...
func (s *States) UpdateWithTs(newState State, ts time.Time) {
	...
	if index >= 0 {
		...
		s.log.Debugf("prospector", "New state added for %s", newState.ZoneId)
	}
}
...
func (s *States) Cleanup() (int, int) {
	...
	for i := 0; i < L; {
		...
		if state.TTL == 0 || expired {
			if !state.Finished {
				s.log.Errorf("State for %s should have been dropped, but couldn't as state is not finished.", state.ZoneId)
				i++
				continue
			}

			delete(s.idx, state.ID())
			s.log.Debugf("state", "State removed for %v because of older: %v", state.ZoneId, state.TTL)
